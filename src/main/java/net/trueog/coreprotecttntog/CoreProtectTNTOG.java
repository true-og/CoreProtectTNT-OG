...
public class CoreProtectTNTOG extends JavaPlugin implements Listener {
    ...
    private final Cache<Object, String> probablyCache = CacheBuilder.newBuilder().expireAfterAccess(1, TimeUnit.HOURS)
    ...
    public void onBlockPlaceOnHanging(BlockPlaceEvent event) {
        ...
        // We can't check the hanging in this event, may cause server lagging, just
        // store it
    ...
    public void onClickItemFrame(PlayerInteractEntityEvent e) { // Add item to item-frame or rotating
        ...
        if (itemFrame.getItem().getType().isAir()) { // Probably put item now
            ...
            if (!putIn.getType().isAir()) {
                ...
                // Put in item
                api.logPlacement("#additem-" + e.getPlayer().getName(), e.getRightClicked().getLocation(),
                        putIn.getType(), null);
                return;
        ...
        api.logRemoval("#rotate-" + e.getPlayer().getName(), e.getRightClicked().getLocation(),
                itemFrame.getItem().getType(), null);
        api.logPlacement("#rotate-" + e.getPlayer().getName(), e.getRightClicked().getLocation(),
                itemFrame.getItem().getType(), null);
    ...
    public void onHangingBreak(HangingBreakEvent e) {
        ...
        if (reason != null) {
            ...
            if (mat != null) {
                ...
                api.logRemoval("#" + e.getCause().name() + "-" + reason, hangingPosBlock.getLocation(),
                        Material.matchMaterial(e.getEntity().getType().name()), null);
    ...
    public void onPaintingHit(EntityDamageByEntityEvent e) {
        ...
        if (e.getDamager() instanceof Player) {
            ...
            if (reason != null) {
                ...
                api.logRemoval("#" + e.getDamager().getName() + "-" + reason, itemFrame.getLocation(),
                        itemFrame.getItem().getType(), null);
    ...
    public void onBlockIgnite(BlockIgniteEvent e) {
        ...
        if (e.getIgnitingEntity() != null) {
            ...
            if (e.getIgnitingEntity().getType() == EntityType.PLAYER) {
                ...
                // Don't add it to probablyIgnitedThisTick because it's the simplest case and is
                // logged by Core Protect
                return;
    ...
    public void onBlockBurn(BlockBurnEvent e) {
        ...
        if (e.getIgnitingBlock() != null) {
            ...
            if (sourceFromCache != null) {
                ...
                probablyCache.put(e.getBlock().getLocation(), sourceFromCache);
                api.logRemoval("#fire-" + probablyCache.getIfPresent(e.getIgnitingBlock().getLocation()),
                        e.getBlock().getLocation(), e.getBlock().getType(), e.getBlock().getBlockData());
            ...
            if (track != null) {
                ...
                if (creeperTarget != null) {
                    ...
                    for (Block block : blockList) {
                        ...
                        api.logRemoval("#creeper-" + creeperTarget.getName(), block.getLocation(), block.getType(),
                                block.getBlockData());
                ...
                if (entry.getKey() instanceof Location loc) {
                    ...
                    if (loc.getWorld().equals(blockCorner.getWorld()) && loc.distance(blockCorner) < 1) {
                        ...
                        for (Block block : blockList) {
                            ...
                            api.logRemoval("#tntminecart-" + entry.getValue(), block.getLocation(), block.getType(),
                                    block.getBlockData());
